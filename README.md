# ⚽ SportPanther App

## 🇧🇷 Sobre o Projeto

O **SportPanther App** é uma aplicação desenvolvida como Trabalho de Conclusão de Curso (TCC), com o objetivo de incentivar e ampliar a participação feminina no futebol. A plataforma conecta jogadoras e facilita o acesso a locais públicos para prática esportiva, promovendo inclusão, visibilidade e fortalecimento do futebol feminino.

## 🇺🇸 About the Project

**SportPanther App** is an application developed as a Final Course Project, aiming to encourage and expand women's participation in football (soccer). The platform connects players and facilitates access to public spaces for sports practice, promoting inclusion, visibility, and strengthening women's football.

---

## 🚀 Funcionalidades | Features

### 🇧🇷 Funcionalidades

* 👥 Conexão entre jogadoras
* 📍 Localização de espaços públicos
* 📢 Incentivo à participação feminina
* 🤝 Promoção de inclusão e acessibilidade 

### 🇺🇸 Features

* 👥 Connection between players
* 📍 Location of public spaces
* 📢 Encouragement of women's participation
* 🤝 Promotion of inclusion and accessibility

---

## 🛠️ Tecnologias | Technologies

* JavaScript

---

## 📚 Contexto | Context

### 🇧🇷

Projeto desenvolvido como Trabalho de Conclusão de Curso, com foco em impacto social por meio da tecnologia.

### 🇺🇸

Project developed as a Final Course Project, focusing on social impact through technology.

---

## 🔒 Licença | License

### 🇧🇷

© 2022 Gustavo Dornellas, Bren-o, GabrielBismark. Todos os direitos reservados.
Este projeto não pode ser copiado, modificado ou utilizado sem autorização prévia.

### 🇺🇸

© 2022 Gustavo Dornellas, Bren-o, GabrielBismark. All rights reserved.
This project may not be copied, modified, or used without prior authorization.

---

## 👨‍💻 Autor | Author

* **Gustavo Dornellas**
* **Bren-o**
* **GabrielBismark**

---
